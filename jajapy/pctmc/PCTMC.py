...
class PCTMC(Parametric_Model):
    ...
    def __init__(
        ...
        matrix: ndarray,
        labelling: list,
        transition_expr: list,
        parameter_values: dict,
        parameter_indexes: list,
        parameter_str: list,
    ...
    def e(self, s: int) -> float:
        ...
        raise ValueError(
            "cannot compute the exit rate of non-instantiated state " + str(s)
        )
    ...
    def tau(self, s1: int, s2: int, obs: str) -> float:
        ...
        try:
            ...
            print(
                "WARN: cannot compute the exit-rate of non-instantiated state "
                + str(s1)
            )
    ...
    def expected_time(self, s: int) -> float:
        ...
        try:
            ...
            print(
                "WARN: cannot compute the expected time of non-instantiated state "
                + str(s)
            )
    ...
    def _stateToString(self, state: int) -> str:
        ...
        if self.isInstantiated(state):
            res += (
                "Exepected waiting time: "
                + str(round(self.expected_time(state), 5))
                + "\n"
            )
        for j in range(len(self.matrix[state])):
            if self.matrix[state][j] != 0:
                ...
                if (
                    self.isInstantiated(state, j)
                    and len(self.involvedParameters(state, j)) > 0
                ):
    ...
    def next(self, state: int) -> tuple:
        ...
        exps = []
        for exp_lambda in [
            self.transitionValue(state, j) for j in range(self.nb_states)
        ]:
            if exp_lambda == 0.0:
                exps.append(inf)
            else:
    ...
    def logLikelihood(self, traces: Set) -> float:
        ...
        if not self.isInstantiated():
            raise ValueError(
                "Cannot compute the loglikelihood of a set under a non-instantiated model."
            )
    ...
    def savePrism(self, file_path: str) -> None:
        ...
        f.write(
            ...
            + str(self.nb_states)
            + "] init "
            + str(where(self.initial_state == 1.0)[0][0])
            + ";\n\n"
        )
                ...
                for j in range(len(m.parameter_indexes)):
                    ...
                    while k < len(m.parameter_indexes[j]):
                        if i in m.parameter_indexes[j][k]:
                            m.parameter_indexes[j] = (
                                m.parameter_indexes[j][:k]
                                + m.parameter_indexes[j][k + 1 :]
                            )
                        else:
...
def synchronousComposition2PCTMCs(
    m1: PCTMC, m2: PCTMC, name: str = "unknown_composition"
) -> PCTMC:
    """
            ...
            for j in m2_sids:
                if m2.matrix[j, j] != 0:
                    ...
                    if not expr in trans_expr:
                        trans_expr.append(expr)
                    matrix[
                        get_state_index(i, j), get_state_index(i, j)
                    ] = trans_expr.index(expr)
    sync_trans = []
