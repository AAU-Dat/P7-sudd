...
def stormpyModeltoJajapy(h, actions_name: list = [], from_prism=False):
    ...
    if add_init_state != None:
        ...
        if ty == 2:
            ...
            matrix = concatenate(
                (matrix, zeros((matrix.shape[0], matrix.shape[1], 1))), axis=2
            )
        elif ty == 1 or ty == 0:
...
def jajapyModeltoStormpy(h):
    ...
    if type(h) == MDP:
        ...
        raise TypeError(
            str(type(h)) + " cannot be translated to a stormpy sparse model."
        )
...
def MDPtoStormpy(h):
    ...
    transition_matrix = reshape(
        transition_matrix.flatten(), (h.nb_states * nb_actions, h.nb_states)
    )
    transition_matrix = st.build_sparse_matrix(
        transition_matrix, [nb_actions * i for i in range(h.nb_states)]
    )
    ...
    reward_models["nb_executed_actions"] = st.SparseRewardModel(
        optional_state_action_reward_vector=action_reward
    )
    components = st.SparseModelComponents(
...
def MCtoStormpy(h):
    ...
    components = st.SparseModelComponents(
        transition_matrix=transition_matrix, state_labeling=state_labelling
    )
