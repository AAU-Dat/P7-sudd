...
class IOFPTA_state:
    ...
    def transitionChange(self, action: str, old_state: int, new_state: int) -> None:
        ...
        self.transitions[action][1][
            self.transitions[action][1].index(old_state)
        ] = new_state
...
class IOFPTA:
        ...
        for a in self.actions:
            for o in self.observations:
                ...
                if not self.compatible(
                    self.successor(s1, a, o), self.successor(s2, a, o), epsilon
                ):
                    return False
        return True
    ...
    def hoeffding(self, s1: int, s2: int, a: str, o: str, epsilon: float) -> bool:
        ...
        return abs((f1 / n1) - (f2 / n2)) < (sqrt(1 / n1) + sqrt(1 / n2)) * sqrt(
            log(2 / epsilon) / 2
        )
            ...
            for o in self.observations:
                ...
                if succ2 != None:
                    ...
                    if succ1 != None:
                        ...
                        self.states[s1].transitionsAdd(
                            a, self.states[s2].getTransitionProb(a, succ2), succ2
                        )
...
class IOAlergia:
        ...
        for s1 in range(len(states_sorted)):
            ...
            while s2 < len(states_sorted):
                if len(states_sorted[s2].label) < len_s1 + 2:  # too short
                    ...
                    states_sorted[s2].label[:-2] != states_sorted[s1].label
                ):  # not same prefix
                    s2 += 1
                else:  # OK
    ...
    def fit(
        ...
        if stormpy_output and not stormpy_installed:
            print(
                "WARNING: stormpy not found. The output model will not be a stormpy sparse model"
            )
