...
class MDP(Base_MC):
    ...
    def __init__(
        self, matrix: ndarray, labelling: list, actions: list, name: str = "unknown_MDP"
    ):
        """
        Create a MDP.
            ...
            for a in range(self.nb_actions):
                if not checkProbabilities(matrix[i][a]):
                    ...
                    msg += (
                        ...
                        + " executing action "
                        + self.actions[a]
                        + " should be 1.0 or 0.0, here it's "
                        + str(matrix[i][a].sum())
                    )
    ...
    def getActions(self, state: int = -1) -> list:
        ...
        if state == -1:
            ...
            return list(
                set(
                    self.actions[i]
                    for i in where(self.matrix[state].sum(axis=1) > 0.0)[0]
                )
            )
    ...
    def _logLikelihood_oneproc(self, sequences: Set) -> float:
        ...
        for seq in range(len(sequences_sorted)):
            sequence_actions = [
                sequences_sorted[seq][i + 1]
                for i in range(0, len(sequences_sorted[seq]) - 1, 2)
            ]
            sequence_obs = [
                sequences_sorted[seq][i]
                for i in range(0, len(sequences_sorted[seq]) - 1, 2)
            ]
            ...
            if seq > 0:
                while common < min(len(sequences_sorted[seq - 1]), len(sequence)):
                    ...
                    common += 1
            common = int(common / 2)
            alpha_matrix = self._updateAlphaMatrix(
                sequence_obs, sequence_actions, common, alpha_matrix
            )
    ...
    def _updateAlphaMatrix(
        ...
        if diff_size < 0:  # alpha_matrix too small
            ...
            alpha_matrix = vsplit(
                alpha_matrix, [len(alpha_matrix) - diff_size, self.nb_states]
            )[0]
        for k in range(common, len(sequence_obs)):
            for s in range(self.nb_states):
                p = array(
                    [
                        self.tau(ss, sequence_actions[k], s, sequence_obs[k])
    ...
    def _computeAlphas(self, sequence: list, times: int) -> float:
        ...
        for k in range(0, len_seq - 1, 2):
            ...
            for s in range(self.nb_states):
                p = array(
                    [
                        self.tau(ss, sequence[k + 1], s, sequence[k])
                        for ss in range(self.nb_states)
                    ]
                )
...
def createMDP(
    transitions: list, labelling: list, initial_state, name: str = "unknown_MDP"
) -> MDP:
    """
