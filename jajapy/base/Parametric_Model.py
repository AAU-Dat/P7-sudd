...
class Parametric_Model(Model):
    ...
    def __init__(
        ...
        initial_state = [
            1.0 / self.labelling.count("init") if i == "init" else 0.0
            for i in self.labelling
        ]

        if len(parameter_indexes) != len(parameter_str):
            raise ValueError(
                "Length of parameter_indexes and parameter_str must be equal."
            )
        self.nb_parameters = len(parameter_indexes)
        if max(matrix.flatten()) >= len(transition_expr):
            msg = (
                "Transition "
                + str(max(matrix.flatten()))
                + " found in matrix while length "
            )
            msg += "of transition_expr is " + str(len(transition_expr))
            raise ValueError(msg)
        if min(matrix.flatten()) < 0:
            raise ValueError(
                "Transition " + str(min(matrix.flatten())) + " found in matrix"
            )
    ...
    def isInstantiated(
        self, state1: int = None, state2: int = None, param: str = None
    ) -> bool:
        """
    ...
    def __str__(self) -> str:
        ...
        if 1.0 in self.initial_state:
            res += (
                "Initial state: s" + str(where(self.initial_state == 1.0)[0][0]) + "\n"
            )
        else:
            res += "Initial state: "
            for i in range(len(self.matrix)):
                if self.initial_state[i] >= 0.001:
                    res += (
                        ...
                        + str(i)
                        + ": "
                        + str(round(self.initial_state[i], 3))
                        + ", "
                    )
            res = res[:-2] + "\n"
    ...
    def _checkStateIndex(self, s: int) -> None:
        ...
        if s < 0:
            ...
            raise IndexError(
                "This model contains only " + str(self.nb_states) + " states"
            )
