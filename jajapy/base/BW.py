...
class BW:
    ...
    def fit(
        ...
        min_exit_rate_time: int = None,
        max_exit_rate_time: int = None,
        self_loop: bool = None,
        ...
        fixed_parameters: ndarray = False,
        update_constant: bool = True,
        min_val: float = None,
        max_val: float = None,
        processes: int = None,
    ):
        ...
        stormpy_output = self._preparation(
            training_set,
            initial_model,
            nb_states,
    ...
    def _preparation(
        ...
        if self.h == None:
            ...
            if (
                (self.training_set.type != self.type_model)
                and (self.training_set.type != 0 and self.type_model != 2)
                and (self.training_set.type != 0 and self.type_model != 4)
                and (self.training_set.type != 0 and self.type_model != 5)
                and (self.training_set.type != 4 and self.type_model != 5)
            ):
                msg = (
                    "the training set and the initial hypothesis are not compatible:\n"
                )
                msg += "the training set is a set of sequences of "
                msg += [
                    "labels",
                    "action-label pairs",
                    "vector of continuous observations",
                    "dwell time-label pairs",
                ][self.training_set.type]
                msg += (
                    "\nand the initial hypothesis is a "
                    + ["MC", "MDP", "HMM", "GoHMM", "CTMC", "PCTMC"][self.type_model]
                )
    ...
    def _bw(
        ...
        with alive_bar(
            alive_parameter, dual_line=True, title=str(pp), disable=progress_bar
        ) as bar:
            while counter < max_it:
                ...
                bar_text = (
                    "   Diff. loglikelihood: "
                    + str(round(currentloglikelihood - prevloglikelihood, 5))
                    + " (>"
                    + str(epsilon)
                    + ")"
                )
                bar_text += "   Av. one iteration (s): " + str(
                    round((datetime.now() - start_time).total_seconds() / counter, 2)
                )
                bar.text = bar_text
                bar()
        ...
        if output_file_prism and (
            self.type_model != HMM_ID and self.type_model != GOHMM_ID
        ):
        ...
        if return_data:
            info = {
                ...
                "training_set_loglikelihood": currentloglikelihood,
            }
            return self.h, info
    ...
    def _processWork_MC(self, sequence, times):
        ...
        for s in range(self.nb_states):
            for ss in range(self.nb_states):
                ...
                num[s, ss] = dot(
                    alpha_matrix[s][:-1] * p * beta_matrix[ss][1:], times / proba_seq
                ).sum()
        ####################
    ...
    def _computeAlphas_MDP(self, sequence: list, sequence_actions: list) -> array:
        ...
        for k in range(len_seq - 1):
            for s in range(self.nb_states):
                p = array(
                    [
                        self._h_tau(ss, sequence_actions[k], s, sequence[k])
                        for ss in range(self.nb_states)
                    ]
                )
    ...
    def _computeBetas_MDP(self, sequence: list, sequence_actions: list) -> array:
        ...
        for k in range(len(sequence) - 2, -1, -1):
            for s in range(self.nb_states):
                p = array(
                    [
                        self._h_tau(s, sequence_actions[k], ss, sequence[k])
                        for ss in range(self.nb_states)
                    ]
                )
    ...
    def _processWork_MDP(self, sequence, times):
        ...
        sequence_obs = [sequence[i] for i in range(0, len(sequence) - 1, 2)] + [
            sequence[-1]
        ]
        ...
        if proba_seq != 0.0:
            ...
            for s in range(self.nb_states):
                for i, a in enumerate(self.actions):
                    arr_dirak = array(
                        [1.0 if t == a else 0.0 for t in sequence_actions]
                    )
                    den[s, i] += dot(
                ...
                for ss in range(self.nb_states):
                    p = array(
                        [
                            self._h_tau(s, a, ss, o)
                            for o, a in zip(sequence_obs[:-1], sequence_actions)
                        ]
                    )
                    for ia, act in enumerate(self.actions):
                        arr_dirak = [1.0 if a == act else 0.0 for a in sequence_actions]
                        num[s, ia, ss] = dot(
                            alpha_matrix[s][:-1] * arr_dirak * beta_matrix[ss][1:] * p,
                            times / proba_seq,
                        ).sum()
            return [den, num, proba_seq, times]
        return False
    ...
    def _processWork_CTMC(self, sequence: list, times: int):
        ...
        if timed:
            ...
            den = (alpha_matrix[:, :-1] * beta_matrix[:, :-1] * times / proba_seq).sum(
                axis=1
            )
            ...
            for ss in range(self.nb_states):
                if not self.fixed_parameters[s, ss]:
                    if timed:
                        p = array(
                            [
                                self._h_l(s, ss, o) * exp(-self._h_e(s) * t)
                                for o, t in zip(obs_seq, times_seq)
                            ]
                        )
                    else:
                        p = array(
                            [self._h_l(s, ss, o) / self._h_e(s) for o in obs_seq]
                        )  # not sure
                    num[s, ss] = dot(
                        alpha_matrix[s][:-1] * p * beta_matrix[ss][1:],
                        times / proba_seq,
                    ).sum()
                else:
                    num[s, ss] = 0.0
    ...
    def _processWork_HMM(self, sequence, times):
        ...
        if proba_seq != 0.0:
            ####################
            den = (alpha_matrix.T[:-1] * beta_matrix.T[:-1] * times / proba_seq).sum(
                axis=0
            )
            ####################
            ...
            for s in range(self.nb_states):
                for ss in range(self.nb_states):
                    ...
                    temp = (
                        alpha_matrix[s][:-1] * beta_matrix[s][:-1] * times / proba_seq
                    )
    ...
    def _update_HMM(self, temp):
        ...
        initial_state = array(
            [lst_init[s].sum() / lst_init.sum() for s in range(self.nb_states)]
        )
    ...
    def _processWork_GoHMM(self, sequence, times):
        ...
        if proba_seq != 0.0:
            ...
            for s in range(self.nb_states):
                num_mu[s] = dot(
                    alpha_matrix[s][:-1] * beta_matrix[s][:-1] * sequence.T,
                    times / proba_seq,
                ).sum(axis=1)
                num_va[s] = dot(
                    alpha_matrix[s][:-1]
                    * beta_matrix[s][:-1]
                    * (sequence - self.h.mu(s)).T ** 2,
                    times / proba_seq,
                ).sum(axis=1)
                for ss in range(self.nb_states):
                    p = array([self._h_tau(s, ss, o) for o in sequence])
    ...
    def _computedAlphas_timed_log_symbolic_PCTMC(self, obs_seq, time_seq):
        return self._computeAlphas_timed_PCTMC(sudd.forwards_log_symbolic, obs_seq, time_seq)
    ...
    def _computedBetas_timed_log_symbolic_PCTMC(self, obs_seq, time_seq):
        return self._computeBetas_timed_PCTMC(sudd.backwards_log_symbolic, obs_seq, time_seq)
    ...
    def _sortParameters(self, fixed_parameters: list):
        ...
        """
        self.a_pis = zeros(
            (self.h.nb_states, self.h.nb_states, len(self.h.parameter_str)),
            dtype=float16,
        )
            ...
            for itrans, trans in enumerate(self.h.transition_expr[1:]):
                if trans.is_real:
                    ...
                    if timed:
                        ...
                        den_cste[itrans + 1] = dot(
                            alpha_matrix[s][:-1] * beta_matrix[s][:-1] * times_seq,
                            times / proba_seq,
                        ).sum()
                    else:
                        p = array([self._h_l(s, ss, o) / self._h_e(s) for o in obs_seq])
                        den_cste[itrans + 1] = dot(
                            alpha_matrix[s][:-1] * beta_matrix[s][:-1],
                            times / proba_seq,
                        ).sum()
                    num_cste[itrans + 1] = dot(
                        alpha_matrix[s][:-1] * p * beta_matrix[ss][1:],
                        times / proba_seq,
                    ).sum()
            ...
            for s, ss in self.h.parameterIndexes(param):
                ...
                if p.sum() > 0.0:
                    if timed:
                        den_cat1[iparam] += dot(
                            alpha_matrix[s][:-1] * beta_matrix[s][:-1] * times_seq,
                            self.c_pis[s, ss] * times / proba_seq,
                        ).sum()
                    else:
                        den_cat1[iparam] += dot(
                            alpha_matrix[s][:-1] * beta_matrix[s][:-1],
                            self.c_pis[s, ss] * times / (self._h_e(s) * proba_seq),
                        ).sum()
                    num_cat1[iparam] += dot(
                        alpha_matrix[s][:-1] * p * beta_matrix[ss][1:],
                        times / proba_seq,
            ...
            for s, ss in self.h.parameterIndexes(param):
                if timed:
                    ...
                    den_cat2[iparam] += dot(
                        ...
                        * self.hval[s, ss]
                        * times
                        / (proba_seq * self._h_e(s)),
                    ).sum()
                num_cat2[iparam] += dot(
                    alpha_matrix[s][:-1] * p * beta_matrix[ss][1:],
                    self.a_pis[s, ss, p_index] * times / proba_seq,
                ).sum()
            ...
            for s, ss in self.h.parameterIndexes(param):
                ...
                if timed:
                    temp[c] += dot(
                        ...
                        * times
                        / proba_seq,
                    ).sum() / (self.h.parameter_values[param] ** c)
                else:
                    temp[c] += dot(
                        alpha_matrix[s][:-1] * beta_matrix[s][:-1],
                        self.a_pis[s, ss, p_index]
                        ...
                        * times
                        / proba_seq,
                    ).sum() / (self._h_e(s) * self.h.parameter_values[param] ** c)
    ...
    def _update_PCTMC(self, temp):
        ...
        parameters = (
            self.parameters_cat[0] + self.parameters_cat[1] + self.parameters_cat[2]
        )
        values = []
        ...
        if self.update_constant:
            ...
            self.h.transition_expr = [
                sympify(i) for i in (num_cste / den_cste).tolist()
            ]
    ...
    def fit_nonInstantiatedParameters(
        self,
        traces,
        initial_model,
        epsilon: float = 0.01,
        ...
        min_val: float = None,
        max_val: float = None,
        compute_alpha_beta_how: ComputeAlphaBetaHow = ComputeAlphaBetaHow.SYMBOLIC
    ) -> dict:
        """
        For PCTMC learning only.
    ...
    def fit_parameters(
        ...
        while s < initial_model.nb_states:
            if initial_model.getLabel(s) not in alphabet:
                ...
                initial_model.labelling = (
                    initial_model.labelling[:s] + initial_model.labelling[s + 1 :]
                )
                ...
                for j in range(len(initial_model.parameter_indexes)):
                    ...
                    while k < len(initial_model.parameter_indexes[j]):
                        if s in initial_model.parameter_indexes[j][k]:
                            initial_model.parameter_indexes[j] = (
                                initial_model.parameter_indexes[j][:k]
                                + initial_model.parameter_indexes[j][k + 1 :]
                            )
                        else:
        ...
        while t < l:
            if not (initial_model.matrix == t).any():
                initial_model.transition_expr = (
                    initial_model.transition_expr[:t]
                    + initial_model.transition_expr[t + 1 :]
                )
        ...
        if (compute_alpha_beta_how == ComputeAlphaBetaHow.CLASSIC
            ...
            self._computeAlphas = self._computeAlphas_timed
            self._computeBetas = self._computeBetas_timed
        elif (compute_alpha_beta_how == ComputeAlphaBetaHow.NUMERIC
                and not self.training_set.type == 4):
        ...
        elif (compute_alpha_beta_how == ComputeAlphaBetaHow.MATRIX_NUMERIC
                and not self.training_set.type == 4):
        ...
        elif (compute_alpha_beta_how == ComputeAlphaBetaHow.SYMBOLIC
                and not self.training_set.type == 4):
        ...
        elif (compute_alpha_beta_how == ComputeAlphaBetaHow.SYMBOLIC_LOG_SEMIRING
                and not self.training_set.type == 4):
            self._computeAlphas = self._computedAlphas_untimed_log_symbolic_PCTMC
            self._computeBetas = self._computedBetas_untimed_log_symbolic_PCTMC
        elif (compute_alpha_beta_how == ComputeAlphaBetaHow.SYMBOLIC_LOG_SEMIRING
                and self.training_set.type == 4):
        ...
        returned = self._bw(
            max_it, pp, epsilon, False, False, verbose, False, return_data
        )
    ...
    def _getInitialModel(self, initial_model, nb_states: int):
        if initial_model == None:
            if nb_states == None:
                raise RuntimeError(
                    "initial_state or the nb_states must be given. Here none of them is."
                )
            else:
                self.h = None
        ...
        try:
            ...
            if not self.stormpy_installed:
                raise RuntimeError(
                    "the initial model is a Storm model and Storm is not installed on the machine"
                )
    ...
    def _createInitialModel(
        ...
        if self.training_set.type == 0:
            ...
            if min_exit_rate_time == None or max_exit_rate_time == None:
                raise RuntimeError(
                    "The minimum and maximum exit rate must be given if the initial hypothesis is not provided."
                )
    ...
    def _computeAlphas(self, sequence: list, dtype=False) -> array:
        ...
        for k in range(len_seq):
            for s in range(self.nb_states):
                p = array(
                    [self._h_tau(ss, s, sequence[k]) for ss in range(self.nb_states)],
                    dtype=dtype,
                )
                alpha_matrix[k + 1, s] = dot(alpha_matrix[k], p)
        return alpha_matrix.T
    ...
    def _computeBetas(self, sequence: list, dtype=False) -> array:
        ...
        for k in range(len(sequence) - 1, -1, -1):
            for s in range(self.nb_states):
                p = array(
                    [self._h_tau(s, ss, sequence[k]) for ss in range(self.nb_states)],
                    dtype=dtype,
                )
                beta_matrix[k, s] = dot(beta_matrix[k + 1], p)
        return beta_matrix.T
