...
class Set:
    ...
    def __init__(self, sequences: list, times: list = None, t: int = None) -> None:
        ...
        if t != None:
            ...
            if (
                type(sequences[0][1]) == float64 or type(sequences[0][1]) == float
            ) and type(sequences[0][0]) == str:
    ...
    def isEqual(self, set2) -> bool:
        ...
        if len(self.sequences) != len(set2.sequences):
            return False
        if len([i for i in self.sequences if i in set2.sequences]) != len(
            set2.sequences
        ):
            return False
    ...
    def getActionsObservations(self) -> list:
        ...
        for seq in range(len(self.sequences)):
            actions += [
                self.sequences[seq][i + 1]
                for i in range(0, len(self.sequences[seq]) - 1, 2)
            ]
            observations += [
                self.sequences[seq][i]
                for i in range(0, len(self.sequences[seq]) - 1, 2)
    ...
    def addSet(self, s2):
        ...
        if self.type != s2.type:
            raise TypeError(
                "The two sets should have been generated by the same"
