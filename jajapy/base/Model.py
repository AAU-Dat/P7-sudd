...
class Model:
    ...
    def _updateAlphaMatrix(
        self, sequence: list, common: int, alpha_matrix: list
    ) -> array:
        """
        ...
        for k in range(common, len(sequence)):
            for s in range(self.nb_states):
                p = array(
                    [self.tau(ss, s, sequence[k]) for ss in range(len(self.matrix))]
                )
    ...
    def _initAlphaMatrix(self, len_seq: int) -> array:
        ...
        alpha_matrix = append(self.initial_state, zero_arr).reshape(
            len_seq + 1, self.nb_states
        )
        return alpha_matrix
    ...
    def _computeAlphas(self, sequence: list, times: int) -> float:
        ...
        for k in range(len_seq - 1):
            ...
            for s in range(self.nb_states):
                p = array(
                    [self.tau(ss, s, sequence[k]) for ss in range(self.nb_states)]
                )
