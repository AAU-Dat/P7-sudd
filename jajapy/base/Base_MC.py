...
class Base_MC(Model):
    ...
    def updateLabelling(self, new_labels: list) -> None:
        ...
        if "init" in new_labels:
            ...
            print(
                "No initial state given: at least one state must be labelled by 'init'."
            )
...
def labelsForRandomModel(nb_states: int, labelling: list, sseed: int = None) -> list:
    ...
    if sseed != None:
        seed(sseed)
    labelling = labelling[: min(len(labelling), nb_states)] + choices(
        labelling, k=nb_states - len(labelling)
    )
