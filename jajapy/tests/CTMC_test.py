...
class CTMCTestclass(unittest.TestCase):
    def test_CTMC_initial_state(var):
        ...
        var.assertEqual(
            ctmc.getLabel(int(where(ctmc.initial_state == 1.0)[0][0])), "init"
        )
    ...
    def test_CTMC_state(var):
        ...
        var.assertEqual(set(m1.getAlphabet()), set(["red", "yellow", "blue", "init"]))
    ...
    def test_BW_CTMC(var):
        ...
        training_set = loadSet(
            "jajapy/tests/materials/ctmc/training_set_timed_CTMC.txt"
        )
        ...
        var.assertAlmostEqual(
            output_expected.logLikelihood(test_set),
            output_gotten.logLikelihood(test_set),
        )

        training_set = loadSet(
            "jajapy/tests/materials/ctmc/training_set_nontimed_CTMC.txt"
        )
        output_expected = loadCTMC(
            "jajapy/tests/materials/ctmc/output_nontimed_CTMC.txt"
        )
        output_gotten = BW().fit(training_set, initial_model, stormpy_output=False)
        test_set = m1.generateSet(10000, 10)
        var.assertAlmostEqual(
    ...
    def test_sync_composition(var):
        output_gotten = synchronousCompositionCTMCs(
            modelCTMC_car_tl(), modelCTMC_ped_tl()
        )
