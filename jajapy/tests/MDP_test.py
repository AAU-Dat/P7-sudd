...
class MDPTestclass(unittest.TestCase):
    def test_MDP_initial_state(var):
        ...
        var.assertEqual(
            mdp.getLabel(int(where(mdp.initial_state == 1.0)[0][0])), "init"
        )
        ...
        var.assertTrue(
            (mdp.matrix[-1][0] == array([0.3, 0.0, 0.0, 0.2, 0.5, 0.0, 0.0])).all()
        )
    ...
    def test_MDP_state(var):
        ...
        var.assertEqual(set(m.getLabel(0)), set("R"))
        var.assertEqual(set(m.getActions(0)), set(["m", "s"]))
    ...
    def test_MDP_observations_actions(var):
        var.assertEqual(set(m.getAlphabet()), set(["L", "R", "HIT", "OK", "init"]))
        var.assertEqual(set(m.getActions()), set(["m", "s"]))
    ...
    def test_MDP_logLikelihood(var):
        ...
        set2 = Set(
            [["init", "m", "R", "s", "R", "m", "R", "s", "OK", "m", "OK", "m", "OK"]],
            [2],
            t=1,
        )
