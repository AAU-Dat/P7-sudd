...
class GoHMM(Base_HMM):
    ...
    def next_obs(self, s: int) -> list:
        ...
        """
        return [
            numpy.random.normal(parameter[0], parameter[1], 1)[0]
            for parameter in self.output[s]
        ]
    ...
    def _stateToString(self, state: int) -> str:
        ...
        for n in range(self.nb_distributions):
            res += (
                ...
                + str(n + 1)
                + ": "
                + str(round(self.output[state][n][0], 4))
                + "\n"
            )
            res += (
                "std "
                + str(n + 1)
                + ": "
                + str(round(self.output[state][n][1], 4))
                + "\n"
            )
        return res
...
def GoHMM_random(
    ...
    for s in range(nb_states):
        ...
        p2 = [
            [
                round(random.uniform(min_mu, max_mu), 3),
                round(random.uniform(min_sigma, max_sigma), 3),
            ]
            for i in range(nb_distributions)
        ]
        p2 = array(p2)
        output.append(p2)
    matrix = array(matrix)
...
def createGoHMM(
    transitions: list, output: list, initial_state: str, name: str = "unknown_GoHMM"
) -> GoHMM:
    """
