...
class MC(Base_MC):
    def __init__(
        self, matrix: ndarray, labelling: list, name: str = "unknown_MC"
    ) -> None:
        """
        Creates an MC.
        ...
        for j in range(len(self.matrix[state])):
            if self.matrix[state][j] > 0.0001:
                res += (
                    ...
                    + str(j)
                    + " : "
                    + str(round(self.matrix[state][j], 5))
                    + "\n"
                )
        return res
...
def createMC(
    transitions: list, labelling: list, initial_state, name: str = "unknown_MC"
) -> MC:
    """
    ...
    if type(initial_state) == int:
        ...
        if len(initial_state) != nb_states:
            raise ValueError(
                "The size of the initial_state list must be equal to the number of states"
            )
