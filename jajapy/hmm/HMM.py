    ...
    def __init__(self, matrix, output, alphabet, initial_state, name="unknown_HMM"):
        ...
        for i in range(self.nb_states):
            if not checkProbabilities(output[i]):
                raise ValueError(
                    "The probability to generate an observation in state",
                    i,
                    "should be 1.0, here it's",
                    output[i].sum(),
                )
...
def HMM_random(
    ...
    seed()
    return HMM(
        matrix, output, alphabet, init, "HMM_random_" + str(nb_states) + "_states"
    )


def createHMM(
    transitions: list, emission: list, initial_state, name: str = "unknown_HMM"
) -> HMM:
    """
