...
def testing(m, seq, steps=5):
    ...
    for i in range(8):
        axs[i % 2][i // 2].plot(
            range(1, len(seq) + 1), [seq[j][i] for j in range(len(seq))], c="b"
        )
        axs[i % 2][i // 2].plot(
            range(steps, len(seq) + 1),
            [seq[steps - 1][i]] + [forecast[j][i] for j in range(len(forecast))],
            c="r",
        )
...
def example_8():
    ...
    initial_hypothesis = ja.GoHMM_random(
        nb_states=15,
        nb_distributions=8,
        min_mu=-5.0,
        max_mu=5.0,
