...
def example_4():
    ...
    training_set = original_model.generateSet(
        1000, 10, scheduler=ja.UniformScheduler(actions)
    )

    # INITIAL HYPOTHESIS GENERATION
    # ------------------------------
    initial_hypothesis = ja.MDP_random(
        nb_states=16, labelling=labels, actions=actions, random_initial_state=False
    )
    initial_hypothesis.labelling = original_model.labelling

    # LEARNING
    # ---------
    output_model_active = ja.Active_BW_MDP().fit(
        training_set,
        lr=0,
        nb_iterations=10,
        sul=original_model,
        nb_sequences=100,
        initial_model=initial_hypothesis,
    )
    ...
    for formula in formulas:
        ...
        result_output_active = stormpy.check_model_sparse(
            output_model_active, properties[0]
        )
        result_output_active = result_output_active.at(
            output_model_active.initial_states[0]
        )
