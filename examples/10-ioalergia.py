...
def example_10():
    ...
    test_set = original_model.generateSet(
        1000, 10, scheduler=ja.UniformScheduler(actions)
    )

    # IOAlergia learning
    # -----------------
    ioalergia_model = ja.IOAlergia().fit(
        training_set, epsilon=0.5, stormpy_output=False
    )
    ...
    # -------------------------
    print(
        "Loglikelihood for the original model  :",
        original_model.logLikelihood(test_set),
    )
    print(
        "Loglikelihood for IOAlergia output model:",
        ioalergia_model.logLikelihood(test_set),
    )
